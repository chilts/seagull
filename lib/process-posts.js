...
module.exports = function processPosts (ctx, callback) {
  ...
  visitEveryDir(
    ...
    function (url, dir, done) {
      ...
      // firstly, order all posts in this vertex
      ...
      const reversed = dir.posts.slice().reverse()
      if (reversed.length) {
        // figure out how many pages there are
        var numberOfPages = Math.ceil(reversed.length / ctx.cfg.postsPerPage)
        console.log('posts:', reversed.length)
        ...
        for (var i = 0; i < numberOfPages; i++) {
          ...
          // stringify 'i'
          dir.pages['' + i] = reversed.slice(start, end)
        }
      } else {
